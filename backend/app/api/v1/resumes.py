...
async def _parse_resume_data(resume: Resume, *, operation: str) -> None:
    """Update parse fields consistently while keeping LLM failures non-fatal."""
    try:
        resume.parsed_data = await parse_resume(resume.raw_text)
        resume.parse_status = "completed"
        resume.parse_error = None
    except Exception as exc:
        logger.warning("Resume %s failed for %s: %s", operation, resume.id, exc)
...
async def upload_resume(
    ...
    if len(content) > MAX_SIZE:
        raise HTTPException(
            status_code=400, detail=f"文件大小不能超过 {settings.MAX_UPLOAD_SIZE_MB}MB"
        )
    if not content:
    ...
    if (
        ...
        and ALLOWED_TYPES[file.content_type] != detected_type
    ):
    ...
    if auto_parse and raw_text:
        await _parse_resume_data(resume, operation="parse")
        await db.flush()
        await db.refresh(resume)
    ...
    return {
        "code": 201,
        "message": "上传成功"
        + ("，解析完成" if resume.parse_status == "completed" else "，解析失败"),
...
async def get_resume(
    ...
    return {"code": 200, "data": ResumeOut.model_validate(resume).model_dump()}
...
async def reparse_resume(
    ...
    await _parse_resume_data(resume, operation="reparse")
    await db.flush()
    await db.refresh(resume)
