...
from app.agent.interview_workflow import public_interview_state
...
from app.services.interview_archive_service import load_effective_interview_states
...
async def list_users(
    ...
    interview_stats = (
        select(
            ...
            func.max(InterviewSession.created_at).label("last_activity"),
        )
    ...
    report_stats = (
        select(
            ScoreReport.user_id.label("user_id"),
            func.avg(ScoreReport.overall_score).label("avg_score"),
        )
        .group_by(ScoreReport.user_id)
        .subquery()
    )
    page_query = (
        select(
            User,
            func.coalesce(interview_stats.c.interview_count, 0),
            report_stats.c.avg_score,
            interview_stats.c.last_activity,
        )
        .outerjoin(interview_stats, interview_stats.c.user_id == User.id)
        .outerjoin(report_stats, report_stats.c.user_id == User.id)
        .where(*filters)
    ...
    items = []
    for u, interview_count, user_avg, last_activity in users_result.all():
...
async def get_user_detail(
    ...
    states = await load_effective_interview_states(get_graph_runtime(), list(sessions))
    ...
    for s in sessions:
        ...
        interviews.append({
            "id": s.id,
            "status": public.get("status", s.status),
            "job_title": s.job.title if s.job else None,
            "answered_count": public.get("answered_count", 0),
            "max_turns": public.get("max_turns", (s.config or {}).get("max_turns")),
