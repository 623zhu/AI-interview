...
class InterviewSession(Base):
    ...
    job_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("job_positions.id", ondelete="CASCADE"), nullable=False
    )
    ...
    job: Mapped["JobPosition"] = relationship(
        "JobPosition", back_populates="interview_sessions"
    )
    ...
    report: Mapped["ScoreReport | None"] = relationship(
        "ScoreReport",
        back_populates="session",
        uselist=False,
        cascade="all, delete-orphan",
    )
    archive: Mapped["InterviewArchive | None"] = relationship(
        "InterviewArchive",
        back_populates="session",
        uselist=False,
        cascade="all, delete-orphan",
...
class InterviewMessage(Base):
    ...
    )  # question/answer/follow_up/evaluation/system
    question_id: Mapped[str | None] = mapped_column(
        String(36), default=None, index=True
    )
