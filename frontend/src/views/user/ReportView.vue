...
function scheduleReportPolling() {
  stopReportPolling()
  reportPollTimer = setTimeout(() => fetchReport(true), 2000)
}

async function fetchReport(silent = false) {
  if (!silent) loading.value = true
  try {
    const res = await authenticatedFetch(`/api/v1/reports/by-session/${sessionId.value}`)
    if (res.ok) {
      ...
      if (json.code === 200) {
        ...
        report.value = null
        reportStatus.value = json.data?.status === 'failed' ? 'failed' : 'missing'
        reportError.value = json.data?.error || ''
      }
    ...
    if (reportStatus.value === 'pending' || reportStatus.value === 'generating') {
...
async function requestGenerate() {
  retrying.value = true
  try {
    const res = await authenticatedFetch(`/api/v1/reports/generate/${sessionId.value}`, {
      method: 'POST',
    })
    const json = await res.json()
